...
class Network:
    ...
    def _init_graph(self) -> None:
        ...
        # freezing lower D layers for better finetuning
        def lower_D(var):
            frozen_layers = ['4x4', '8x8', '16x16', '32x32']
            if any(s in str(var) for s in frozen_layers) and 'D' in str(var) and 'G' not in str(var):
                return True
            return False
        if 'freezeD' in self.static_kwargs:
            print(' freezing lower D layers from 4x4 to 32x32 [networks.py]')
            self.trainables = OrderedDict((name, var) for name, var in self.vars.items() if var.trainable and not lower_D(var))
            print(' scope', self.scope, '\t own_vars', len(self.own_vars), '\t vars', len(self.vars), '\t trainables', len(self.trainables))
        else:
    ...
    def apply_swa(self, src_net, epoch):
        ...
        scale_moving_average = 1.0 - 1.0 / (epoch + 1)
        scale_new_data = 1.0 - scale_moving_average
