...
def _create_run_dir_local(submit_config: SubmitConfig) -> str:
    ...
    submit_config.run_name = "{0:03d}-{1}".format(submit_config.run_id, submit_config.run_desc)
...
def submit_run(submit_config: SubmitConfig, run_func_name: str, **run_func_kwargs) -> None:
    ...
    submit_config.task_name = "{0}-{1:03d}-{2}".format(submit_config.user_name, submit_config.run_id, submit_config.run_desc)
