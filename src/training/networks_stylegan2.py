...
# Main generator network. Composed of two sub-networks (mapping and synthesis), defined below
def G_main(
...
# Mapping network. Transforms the input latent code (z) to the disentangled latent code (w)
def G_mapping(
    latents_in,                             # First input: Latent vectors (Z) [minibatch, latent_size].
    labels_in,                              # Second input: Conditioning labels [minibatch, label_size].
...
# StyleGAN2 discriminator. Implements skip connections and residual nets, but no progressive growing.
def D_stylegan2(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    images_in.set_shape([None, num_channels, resolution * init_res[0] // 4, resolution * init_res[1] // 4])
    labels_in.set_shape([None, label_size])
    images_in = tf.cast(images_in, dtype)
    labels_in = tf.cast(labels_in, dtype)

    # Building blocks for main layers.
    def fromrgb(x, y, res): # res = 2..res_log2
        with tf.variable_scope('FromRGB'):
            t = apply_bias_act(conv2d_layer(y, fmaps=nf(res-1), kernel=1, impl=impl), act=act, impl=impl)
            return t if x is None else x + t
    def block(x, res): # res = 2..res_log2
        t = x
    ...
    for res in range(res_log2, 2, -1):
        # with tf.variable_scope('%dx%d' % (2**(res-2) * init_res[0], 2**(res-2) * init_res[1])):
        with tf.variable_scope('%dx%d' % (2**res, 2**res)):
            if architecture == 'skip' or res == res_log2:
                x = fromrgb(x, y, res)
            x = block(x, res)
    ...
    # Final layers.
    # with tf.variable_scope('%dx%d' % (init_res[0], init_res[1])):
    with tf.variable_scope('4x4'):
        if architecture == 'skip':
            x = fromrgb(x, y, 2)
    ...
    with tf.variable_scope('Output'):
        # original version
        x = apply_bias_act(dense_layer(x, fmaps=max(labels_in.shape[1], 1)), impl=impl)
        if labels_in.shape[1] > 0:
            x = tf.reduce_sum(x * labels_in, axis=1, keepdims=True)
        """ Diff Augment version
        if label_size > 0:
            x = apply_bias_act(dense_layer(x, fmaps=label_size), impl=impl)
        else:
            x = tf.squeeze(apply_bias_act(dense_layer(x, fmaps=1), impl=impl), axis=1)
        """
    scores_out = x
